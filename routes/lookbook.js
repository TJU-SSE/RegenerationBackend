...
router.get('/select/:id', async (ctx, next) => {
    try {
        ...
        let lookbook = await LookbookService.findOne({id: id});
        if (!lookbook) { ctx.response.body = ResponseService.createErrResponse('Lookbook not found'); return; }
        let ret = await LookbookService.createLookbookViewModel(lookbook);
...
router.post('/create', async (ctx, next) => {
    try {
        ...
        let ret = await LookbookService.create(timestamp, file.path, title, releaseTime, description);
        ctx.response.body = ResponseService.createJSONResponse(ret);
    } catch (e) {
        ctx.response.body = ResponseService.createErrResponse(e);
    }
...
router.post('/updateImg',  async (ctx, next) => {
    try {
        ...
        let lookbook = await LookbookService.findOne({id: id});
        if (!lookbook) { ctx.response.body = ResponseService.createErrResponse('Lookbook not found'); return; }
        let portfolio = await LookbookService.getPortfolio(lookbook);
...
router.post('/update', async (ctx, next) => {
    try {
        ...
        let releaseTime = ctx.request.body.fields.releaseTime || 0;
        let description = ctx.request.body.fields.description || '';
        let ret = await LookbookService.update(lookbook, title, releaseTime, description);
        ctx.response.body = ResponseService.createJSONResponse(ret);
    } catch(e) {
        ctx.response.body = ResponseService.createErrResponse(e);
    }
...
router.post('/delete', async (ctx, next) => {
    try {
        let id = ctx.request.body.id;
        if (!id) { ctx.response.body = ResponseService.createErrResponse('Id not found'); return; }
        let ret = await LookbookService.delete({id: id});
        ctx.response.body = ResponseService.createJSONResponse(ret);
    } catch (e) {
        ctx.response.body = ResponseService.createErrResponse(e);
    }
