...
router.get('/select/:id', async (ctx, next) => {
    try {
        ...
        let campaign = await CampaignService.findOne({id: id});
        if (!campaign) { ctx.response.body = ResponseService.createErrResponse('Campaign not found'); return; }
        let ret = await CampaignService.createCampaignViewModel(campaign);
...
router.post('/create', async (ctx, next) => {
    try {
        ...
        let ret = await CampaignService.create(timestamp, file.path, title, releaseTime, description);
        ctx.response.body = ResponseService.createJSONResponse(ret);
    } catch (e) {
        ctx.response.body = ResponseService.createErrResponse(e);
    }
...
router.post('/updateImg',  async (ctx, next) => {
    try {
        ...
        let campaign = await CampaignService.findOne({id: id});
        if (!campaign) { ctx.response.body = ResponseService.createErrResponse('Campaign not found'); return; }
        let portfolio = await CampaignService.getPortfolio(campaign);
...
router.post('/update', async (ctx, next) => {
    try {
        ...
        let releaseTime = ctx.request.body.fields.releaseTime || 0;
        let description = ctx.request.body.fields.description || '';
        let ret = await CampaignService.update(campaign, title, releaseTime, description);
        ctx.response.body = ResponseService.createJSONResponse(ret);
    } catch(e) {
        ctx.response.body = ResponseService.createErrResponse(e);
    }
...
router.post('/delete', async (ctx, next) => {
    try {
        let id = ctx.request.body.id;
        if (!id) { ctx.response.body = ResponseService.createErrResponse('Id not found'); return; }
        let ret = await CampaignService.delete({id: id});
        ctx.response.body = ResponseService.createJSONResponse(ret);
    } catch (e) {
        ctx.response.body = ResponseService.createErrResponse(e);
    }
